...
def process_pdf_files(pdf_base_folder_path):
    # ==================== FINAL BILINGUAL PDF EXTRACTION FUNCTION ====================
    def extract_data_from_pdf(pdf_path):
        # Using dictionaries for cleaner code
        KEYWORDS_SET_1 = {
            "main": "³èº ³èº t¡à[ºA¡à", "add_header": "ë™àK",
            "sub_del_header": "¤àƒ", "del_header": "J) Ç¡‹¹oã¹",
            "total_line": "³åk¡"
        }
        KEYWORDS_SET_2 = {
            "main": "³èº t¡à[ºA¡à", "add_header": "Î}ì™à\>",
            "sub_del_header": "[¤ìÚà\>", "del_header": "J) Î}ìÅà‹ìo¹",
            "total_line": "ì³ài¡¡"
        }
        
        try:
            with pdfplumber.open(pdf_path) as pdf:
                if not pdf.pages: return {'main': 'Empty PDF', 'add': 0, 'sub_del': 0, 'del': 0}

                pages_to_check = pdf.pages[-3:]; pages_to_check.reverse()

                for page in pages_to_check:
                    text = page.extract_text(x_tolerance=2, y_tolerance=2)
                    
                    if text and (KEYWORDS_SET_1["main"] in text or KEYWORDS_SET_2["main"] in text):
                        ...
                        for line in text.split('\n'):
                            ...
                            if KEYWORDS_SET_1["main"] in line or KEYWORDS_SET_2["main"] in line:
                                nums = re.findall(r'\d+', line)
                                if len(nums) >= 3: main = int(nums[-1])
                            
                            elif KEYWORDS_SET_1["add_header"] in line or KEYWORDS_SET_2["add_header"] in line:
                                in_add, in_del = True, False
                            
                            elif KEYWORDS_SET_1["del_header"] in line or KEYWORDS_SET_2["del_header"] in line:
                                in_del, in_add = True, False
                            
                            if in_add:
                                if KEYWORDS_SET_1["sub_del_header"] in line or KEYWORDS_SET_2["sub_del_header"] in line:
                                    in_sub_del = True
                                elif in_sub_del and (KEYWORDS_SET_1["total_line"] in line or KEYWORDS_SET_2["total_line"] in line):
                                    nums = re.findall(r'\d+', line)
                                    if nums: sub_del = int(nums[-1]); in_sub_del = False
                                elif add == 0 and len(re.findall(r'\d+', line)) >= 3:
                                    nums = re.findall(r'\d+', line)
                                    add = int(nums[-1])

                            if in_del and (KEYWORDS_SET_1["total_line"] in line or KEYWORDS_SET_2["total_line"] in line):
                                nums = re.findall(r'\d+', line)
